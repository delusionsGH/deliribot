...
bot.command("whois", { // looks for the data of a user, YES I KNOW "fetc" IS WEIRD but mbw told me not to use fetch :(
    ...
    fn: async function (reply, [name], _post) {
        ...
        const user = name;
        const id = fetc.uuid;
        const avatarcolor = `#${fetc.avatar_color}`;
        const quote = `\"${fetc.quote}\"`;
        const avatar = fetc.avatar
        log(chalk.blue(`combining data...`));
        const result = `# ***@${user}***\n[link to profile picture](https://uploads.meower.org/icons/${avatar})\n\n**Quote:**\n${quote.replace(/^/gm, "> ")}\n\n**UUID:** "${id}"\n**Avatar's hex code:**  ${avatarcolor}`; // combine it all
        await reply(result);
